...
async function authLocal(path){ const username=el.localUsername.value.trim(); const phone=el.localPhone.value.trim(); const password=el.localPassword.value.trim(); if(!username||!phone||!password) return alert('Заполни username, phone, password'); const r=await fetch(`${AUTH_API_BASE}${path}`,{method:'POST',headers:{'Content-Type':'application/json'},credentials:'include',body:JSON.stringify({username,phone,password})}); const d=await r.json().catch(()=>({})); if(!r.ok) return alert(d.error||'Auth error'); state.user=d.user; profile.nickname=d.user.name; saveProfile(); renderAll(); }
...
el.registerBtn.addEventListener('click',()=>authLocal('/auth/register'));
el.loginBtn.addEventListener('click',()=>authLocal('/auth/login'));
